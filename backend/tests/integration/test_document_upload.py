...
from src.config import TEST_VECTOR_STORE_COLLECTION_NAME, TEST_DATABASE_NAME, MONGO_DB_CONNECTION_STRING
...
async def test_succesful_document_upload_web():
    ...
    response = await upload_to_vector_store(database=test_mongo_database,documents=documents,collection_name=TEST_VECTOR_STORE_COLLECTION_NAME)
